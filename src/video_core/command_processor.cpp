...
static void WriteUniformFloatReg(ShaderRegs& config, Shader::ShaderSetup& setup,
    ...
    if ((float_regs_counter >= 4 && uniform_setup.IsFloat32()) ||
        ...
        Common::Vec4<float24>& uniform = setup.uniforms.f[uniform_setup.index];
...
static void WritePicaReg(u32 id, u32 value, u32 mask) {
        ...
        if (g_state.default_attr_counter >= 3) {
            ...
            if (setup.index < 15) {
                ...
                if (immediate_attribute_id < regs.pipeline.max_input_attrib_index) {
                    ...
                    Shader::ShaderEngine* shader_engine = Shader::GetEngine();
    ...
    case PICA_REG_INDEX(pipeline.command_buffer.trigger[1]): {
        ...
        const u32* head_ptr = (u32*)VideoCore::g_memory->GetPhysicalPointer(
    ...
    case PICA_REG_INDEX(pipeline.trigger_draw_indexed): {
        ...
        if (regs.pipeline.use_gs == PipelineRegs::UseGS::No) {
            const PipelineRegs::TriangleTopology topology = primitive_assembler.GetTopology();
        ...
        if (batch_id == 0) {
            ++batch_id;
            for (CachedVertex& entry : vs_output) {
                entry.batch = 0;
            }
        }
        ...
        const auto VSUnitLoop = [&](u32 thread_id, const u32 num_threads) {
...
void ProcessCommandList(const u32* list, u32 size) {
    ...
    while (g_state.cmd_list.current_ptr < g_state.cmd_list.head_ptr + g_state.cmd_list.length) {
        ...
        const u32 value = *g_state.cmd_list.current_ptr++;
