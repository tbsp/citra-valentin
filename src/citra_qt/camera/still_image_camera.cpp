...
const std::string StillImageCameraFactory::GetFilePath() const {
    ...
    QString filter = QStringLiteral("Supported image files (%1)").arg(temp_filters.join(" "));
    last_path = QFileDialog::getOpenFileName(nullptr, QStringLiteral("Open File"), ".", filter)
                    .toStdString();
    return last_path;
}
